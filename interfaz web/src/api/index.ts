...
export const listSectores = (ciudadLabel: string): Promise<Sector[]> =>
  request(`/sectores?ciudad=${encodeURIComponent(cityId(ciudadLabel))}`)
...
export const listDanos = (ciudadLabel: string): Promise<ReporteDano[]> =>
  request(`/danos?ciudad=${encodeURIComponent(cityId(ciudadLabel))}`)
